...
public class GUIJFrame extends javax.swing.JFrame {
	//sortingJFile used to retrieve the sorting algorithm names which are then automatically inputed as options into the GUI
	private MasterSortingFile sortingFile = new MasterSortingFile();
	...
	private void initComponents() {
		...
		arraySizeJLabel.setFont(new java.awt.Font("Lucida Grande", 0, 12));
		arraySizeJLabel.setText("Array Size");

		sortSpeedJLabel.setFont(new java.awt.Font("Lucida Grande", 0, 12));
		...
		applicationJLabel.setFont(new java.awt.Font("Lucida Grande", 1, 14));
	...
	//reset the array to its initial values WITHOUT re-shuffling
	private void restartSameSortPanel() {
		sortingJPanel.array.restartSameArray();
		initalizeSortPanel();
	}

	//re-initialize counters to their default values (0)
	private void initalizeSortPanel() {
		sortingJPanel.resetJTextFieldCounts();
	}

	//randomize the unsorted array
	private void randomizeSortPanel() {
		sortingJPanel.setSize(sizeSlider.getValue());
		initalizeSortPanel();
	}

	//update the delay to the new value set by the slider
	private void updateSortSpeed() {
		sortingJPanel.setSpeed((speedSlider.getValue()) * 0.5);
	}

	//Initiate the sort sequence (see main() method)
	...
	public static void main(String[] args) {
		...
		while (true) { //check if the sort-button was pressed
			//update delay textfield while the 
			JFrame.delayTextField.setText(JFrame.speedSlider.getValue() * 0.5 + " ms");
			//initiate sorting algorithim
			if (JFrame.isSorting == true) {
				JFrame.sortingJPanel.startSort(JFrame.sortType);
				JFrame.isSorting = false;
			}
			// forcefully re-render the JPanel
			JFrame.validate();
		}
	}
